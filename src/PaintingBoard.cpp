...
PaintingBoard::PaintingBoard(QQuickItem *parent)
    ...
    , m_markId(-1)
{
   ...
   //saveTimer->start();
    ...
    connect(this, &PaintingBoard::sendMessage, backendSync, [=](const QString& msg){
...
void PaintingBoard::clear()
{
    purgePaintElements();
    update();
}

void PaintingBoard::undo()
{
    if(m_elements.size())
    {
...
void PaintingBoard::paint(QPainter *painter)
{
...
bool PaintingBoard::saveImage(const QVariant& var) {
    qInfo() << var;
    qInfo() << var.type();
...
void PaintingBoard::touchEvent(QTouchEvent *event)
{
...
void PaintingBoard::purgePaintElements()
{
    int size = m_elements.size();
    if(size > 0)
    {
...
void PaintingBoard::initMark()
{
    //可能要造一种数据结构?
}

QStringList PaintingBoard::getMarks()
{
...
void PaintingBoard::paintMark(int _markId, const QPointF& pos) {
...
void PaintingBoard::parseAyncMessage(const QString& message) {
